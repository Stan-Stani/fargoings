...
export class DowntownFargoFetcher {
  ...
  async fetchEvents(daysAhead: number = 14, existingEventIds: Set<string> = new Set()): Promise<DowntownFargoEventWithDetails[]> {
    ...
    try {
      ...
      const events: DowntownFargoEvent[] = await response.json();

      // Only fetch location details for new events
      const newEvents = events.filter(e => !existingEventIds.has(`dtf_${e.url.replace('/events/', '')}`));
      const existingEvents = events.filter(e => existingEventIds.has(`dtf_${e.url.replace('/events/', '')}`));

      console.log(`   Found ${events.length} events (${newEvents.length} new, ${existingEvents.length} cached)`);

      const eventsWithDetails: DowntownFargoEventWithDetails[] = [];

      // Fetch locations only for new events
      if (newEvents.length > 0) {
        console.log(`   Fetching location details for ${newEvents.length} new events...`);
      ...
      // Add existing events without fetching (location will be preserved from DB)
      for (const event of existingEvents) {
        eventsWithDetails.push({ ...event, location: undefined });
      }
  ...
  private async fetchEventLocation(eventPath: string): Promise<string | undefined> {
    try {
      ...
      const venueMatch = html.match(/o-details-block__details-copy[^>]*>\s*([^<]+)</);
  ...
  transformToStoredEvent(event: DowntownFargoEventWithDetails): Omit<StoredEvent, 'id' | 'createdAt' | 'updatedAt'> {
