import { FargoUndergroundAPIResponse, FargoUndergroundEvent, StoredEvent } from '../types/event';

export class FargoUndergroundFetcher {
  private readonly baseUrl = 'https://fargounderground.com/wp-json/tribe/events/v1/events';

  async fetchEvents(perPage: number = 100, daysAhead: number = 14): Promise<FargoUndergroundEvent[]> {
    ...
    while (hasMore) {
      ...
      try {
        ...
        const data: FargoUndergroundAPIResponse = await response.json();
        allEvents.push(...data.events);

        console.log(`   Fetched page ${page}/${data.total_pages} (${data.events.length} events)`);
  ...
  transformToStoredEvent(event: FargoUndergroundEvent): Omit<StoredEvent, 'id' | 'createdAt' | 'updatedAt'> {
