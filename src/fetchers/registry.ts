import { getActiveCityFetchFns } from "../cities/fetchers"
import { FetchedEvent } from "../cities/types"
...
export type { FetchedEvent } from "../cities/types"
...
const FETCH_FNS: Record<string, () => Promise<FetchedEvent[]>> =
  getActiveCityFetchFns()

/**
 * Single source of truth for every event source of the active city. Adding a
 * source means one SourceInfo entry in src/cities/<city>/sources.ts plus one
 * fetch closure in src/cities/<city>/fetchers.ts — index.ts and refetch.ts
